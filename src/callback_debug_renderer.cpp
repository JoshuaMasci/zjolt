...
bool IgnoreListBodyDrawFilter::ShouldDraw(const JPH::Body &inBody) const {
  ...
  for (uint32_t i = 0; i < this->ignore_bodies_count; i++) {
    if (inBody.GetID() == this->ignore_bodies[i]) {
      return false;
    }
  }
...
void CallbackDebugRenderer::DrawLine(JPH::RVec3Arg inFrom, JPH::RVec3Arg inTo,
                                     JPH::ColorArg inColor) {
  if (callback_data.draw_line) {
    DrawLineData data{{inFrom.GetX(), inFrom.GetY(), inFrom.GetZ()},
...
void CallbackDebugRenderer::DrawTriangle(
...
void CallbackDebugRenderer::DrawText3D(JPH::RVec3Arg inPosition,
...
JPH::DebugRenderer::Batch CallbackDebugRenderer::CreateTriangleBatch(
  ...
  if (callback_data.create_triangle_mesh) {
    callback_data.create_triangle_mesh(callback_data.ptr, id,
                                       (const Saturn::Triangle *)inTriangles,
                                       inTriangleCount);
  }
  return new CallbackRenderPrimitive(callback_data.ptr, callback_data.free_mesh,
                                     id);
}
...
JPH::DebugRenderer::Batch CallbackDebugRenderer::CreateTriangleBatch(
  ...
  if (callback_data.create_indexed_mesh) {
    callback_data.create_indexed_mesh(callback_data.ptr, id,
                                      (const Saturn::Vertex *)inVertices,
                                      inVertexCount, inIndices, inIndexCount);
  }
  return new CallbackRenderPrimitive(callback_data.ptr, callback_data.free_mesh,
                                     id);
}

void CallbackDebugRenderer::DrawGeometry(
    const JPH::Mat44 &inModelMatrix, const JPH::AABox &inWorldSpaceBounds,
  ...
  if (callback_data.draw_geometry) {
    auto primitive = dynamic_cast<CallbackRenderPrimitive *>(
    ...
    DrawGeometryData data{primitive->GetId(),
                          color_from_u32(inModelColor.mU32),
                          cull_mode,
                          draw_mode,
                          {0.0}};
    storeMat44(inModelMatrix, data.model_matrix);
    callback_data.draw_geometry(callback_data.ptr,
                                data); // TODO: figure out lod
  }
}
