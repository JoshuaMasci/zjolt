...
public:
  IgnoreListBodyDrawFilter(const JPH::BodyID *const *ignore_bodies,
...
private:
  const JPH::BodyID *const *ignore_bodies;
  const uint32_t ignore_bodies_count;
};

class CallbackRenderPrimitive : public JPH::RefTarget<CallbackRenderPrimitive>,
                                public JPH::RefTargetVirtual {
public:
  CallbackRenderPrimitive(void *data, FreeMeshPrimitive free_fn,
                          MeshPrimitive id)
      : data(data), free_fn(free_fn), id(id) {}
  ~CallbackRenderPrimitive() override {
    if (this->free_fn)
      this->free_fn(data, id);
  };

  void AddRef() override { JPH::RefTarget<CallbackRenderPrimitive>::AddRef(); };
  void Release() override {
    JPH::RefTarget<CallbackRenderPrimitive>::Release();
  };
...
public:
  ...
  void DrawGeometry(const JPH::Mat44 &inModelMatrix,
...
constexpr bool validate_compatible_structs() {
  ...
  static_assert(offsetof(Vertex, position) ==
                    offsetof(JPH::DebugRenderer::Vertex, mPosition),
                "Position field offset mismatch");

  static_assert(offsetof(Vertex, normal) ==
                    offsetof(JPH::DebugRenderer::Vertex, mNormal),
                "Normal field offset mismatch");

  static_assert(offsetof(Vertex, uv) ==
                    offsetof(JPH::DebugRenderer::Vertex, mUV),
                "UV field offset mismatch");

  static_assert(offsetof(Vertex, color) ==
                    offsetof(JPH::DebugRenderer::Vertex, mColor),
  ...
  constexpr size_t jph_triangle_vertex_stride =
...
static_assert(validate_compatible_structs(),
              "Struct compatibility validation failed");
